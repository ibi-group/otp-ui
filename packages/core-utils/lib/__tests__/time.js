...
function setTestTime(time) {
  const date = new Date(time); // Log human-readable date to help out human testers.
...
describe("time", () => {
  ...
  describe("getCurrentDate", () => {
    it("should return current date at specified timezone", () => {
      setDefaultTestTime();
      expect((0, _time.getCurrentDate)("America/New_York")).toMatchSnapshot();
    });
  });
  describe("getCurrentTime", () => {
    it("should return time at specified timezone", () => {
      setDefaultTestTime();
      expect((0, _time.getCurrentTime)("America/New_York")).toMatchSnapshot();
    });
  });
});